...
sub new {
  ...
  #   int    int    double  sampler   3d    3d               int int  3d   3d        double double double
  my ($hres, $vres, $psize, $sampler, $eye, $distance_to_vp, $f, $lr, $up, $look_at, $roll, $zoom, $exposure_time, $max_depth) = @_;
  ...
  $this->{_mj_sampler}->map_samples_to_unit_disk();
...
sub render_scene {
  ...
  my $dp = new Triple(0, 0, 0);
  my $lp = new Triple(0, 0, 0);
  ...
  for (my $r = 0; $r < $this->{_vres}; $r++) {
    for (my $c = 0; $c < $this->{_hres}; $c++) {
      ...
      for (my $j = 0; $j < $this->{_sampler}->num_samples(); $j++) {
        ...
        $dp = $this->{_mj_sampler}->sample_unit_disk();
        $lp = $dp * $this->{_lens_radius};
        
        $ray->origin($this->{_eye} + $lp->fst() * $this->{_u} + $lp->snd() * $this->{_v});
	      $ray->direction($this->ray_direction($pp, $lp));
...
sub ray_direction {
  ...
  my $p = new Triple(0, 0, 0);
  $p->fst($pp->fst() * $this->{_f} / $this->{_d});
  $p->snd($pp->snd() * $this->{_f} / $this->{_d});
  
  my $dir = (($pp->fst() - $lp->fst()) * $this->{_u}) +
            (($pp->snd() - $lp->snd()) * $this->{_v}) -
			      ($this->{_f} * $this->{_w});
  $dir = $dir->normalize();
  return $dir;
}
